  ...
  return (
    <div ref={rootRef} className={cn("relative", className)}>
      <button
        ...
        className={cn(
          "flex h-12 w-full cursor-pointer items-center justify-between rounded-[10px] border border-[var(--border-inputfield)] bg-[var(--background-white)] px-4 text-left text-[14px] text-[var(--text-primary)] focus:outline-none focus:ring-1 focus:ring-[var(--border-primary)]",
        ...
        >
          {options.map((option) => (
            <button
              ...
              className={cn(
                "flex w-full cursor-pointer items-center rounded-[8px] px-3 py-2 text-left text-[14px] transition-colors",
                value === option.value

...
function SummaryField({ label, value }) {
  return (
    <div>
      <p className="text-xs font-normal text-[var(--test-subtext)]">{label}</p>
    ...
    <div className="rounded-[14px] border border-[var(--border-disabled)] bg-[var(--background-white)] p-6">
      ...
      <div className="mt-5 space-y-6">
        ...
        <div className="grid grid-cols-1 gap-5 md:grid-cols-2 xl:grid-cols-4">
          ...
          <SummaryField label="Total Question Set" value={info.totalQuestionSet} />
          <SummaryField label="Duration Per Slots (Minutes)" value={info.duration} />
        </div>

        <div className="grid grid-cols-1 gap-5 md:grid-cols-3">
          <SummaryField label="Question Type" value={info.questionType} />
          <SummaryField label="Start Time" value={info.startTime} />
          <SummaryField label="End Time" value={info.endTime} />
        </div>
      </div>
    </div>
  );
